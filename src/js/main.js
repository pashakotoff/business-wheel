...
function generateChecklistPDF() {
  ...
  var items = [
    ...
    'Вы работаете В бизнесе, а не НАД бизнесом — 80% времени на оперативку'
  ];

  var itemsHTML = items.map(function(item, i) {
    return '<div style="display:flex;align-items:flex-start;background:#0C1526;border:1px solid #1E3A5F;border-radius:8px;padding:10px 14px;margin-bottom:7px;">' +
      '<div style="width:16px;height:16px;min-width:16px;border:1.5px solid #334155;border-radius:4px;margin-top:3px;"></div>' +
      '<span style="color:#F59E0B;font-weight:700;font-size:13px;margin:0 10px;min-width:22px;">' + (i + 1) + '.</span>' +
      '<span style="color:#E2E8F0;font-size:13px;line-height:1.5;">' + item + '</span>' +
      '</div>';
  }).join('');

  var bcMark = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="64.55 298.98 162.46 162.46" width="52" height="52" style="display:block;">' +
    '<rect x="64.55" y="298.98" width="162.46" height="162.46" rx="14" fill="#131C2E"/>' +
    '<g fill="#FFFFFF">' +
    '<polygon points="204.86 345.79 204.86 374.47 190.8 374.47 190.8 354.07 164.97 339.2 164.97 421.28 190.8 406.39 190.8 385.89 204.86 385.89 204.86 414.64 198.73 418.18 164.92 437.65 157.95 441.66 150.91 445.72 150.91 314.7 204.86 345.79"/>' +
    '<path d="M140.9,314.7l-54.19,31.08v68.85s14.07,7.92,14.07,7.92l26.05,15.04,14.07,8.12v-59.84l-6.24-3.6-3.68-2.11,9.92-5.7v-59.76ZM126.83,421.31l-26.05-14.92v-20.51h11.95l14.1,8.12v27.31ZM126.83,365.4l-15.74,9.06h-10.32v-20.39l26.05-14.87v26.2Z"/>' +
    '</g></svg>';

  var el = document.createElement('div');
  el.style.cssText = 'position:fixed;top:0;left:0;z-index:99999;width:794px;background:#070B14;font-family:Inter,Arial,sans-serif;';
  el.innerHTML =
    '<div style="position:relative;background:#070B14;overflow:hidden;">' +
      ...
      '<div style="padding:28px 36px 44px;">' +
        ...
        '<div style="display:flex;align-items:center;gap:14px;margin-bottom:28px;">' +
          bcMark +
          '<div>' +
            '<div style="font-size:14px;font-weight:600;color:#F1F5F9;letter-spacing:2px;margin-bottom:3px;">BUSINESS COMMANDOS</div>' +
            '<a href="https://businesscommandos.ru" style="font-size:11px;color:#F59E0B;text-decoration:none;">businesscommandos.ru</a>' +
          '</div>' +
        '</div>' +

        // Title
        '<div style="font-size:30px;font-weight:700;color:#F1F5F9;line-height:1.2;margin-bottom:3px;">10 признаков бизнеса</div>' +
        '<div style="font-size:30px;font-weight:700;color:#F59E0B;line-height:1.2;margin-bottom:14px;">на ручном управлении</div>' +

        // Subtitle
        '<div style="font-size:13px;color:#94A3B8;padding-left:12px;border-left:3px solid #F59E0B;margin-bottom:18px;">Проверьте, сколько из этих признаков — про ваш бизнес:</div>' +

        '<div style="height:1px;background:#1E3A5F;margin-bottom:14px;"></div>' +

        itemsHTML +

        '<div style="height:1px;background:#1E3A5F;margin:14px 0;"></div>' +

        // Result
        '<div style="background:#131C2E;border:1px solid #1E3A5F;border-radius:10px;padding:14px 18px;margin-bottom:16px;">' +
          '<div style="font-size:13px;font-weight:600;color:#F59E0B;margin-bottom:4px;">Если отмечено 5+ пунктов — ваш бизнес на ручном управлении.</div>' +
          '<div style="font-size:12px;color:#94A3B8;">Это не приговор, но потолок роста. И его можно пробить.</div>' +
        '</div>' +

        // CTA
        '<div style="background:#131C2E;border:1px solid #1E3A5F;border-left:4px solid #F59E0B;border-radius:10px;padding:18px 20px;margin-bottom:22px;">' +
          '<div style="font-size:18px;font-weight:700;color:#F1F5F9;margin-bottom:8px;">Узнали себя? Это можно исправить.</div>' +
          '<div style="font-size:12px;color:#94A3B8;margin-bottom:14px;">Посмотрите ваши результаты теста и запишитесь на бесплатный 30-минутный разбор с Павлом Котовым. Разберём слабые зоны и найдём точки роста.</div>' +
          '<div style="background:#F59E0B;border-radius:6px;padding:11px;text-align:center;font-size:14px;font-weight:700;color:#070B14;">Откройте ваши результаты и запишитесь на разбор →</div>' +
        '</div>' +

        // Footer
        '<div style="display:flex;justify-content:space-between;align-items:center;">' +
          '<a href="https://businesscommandos.ru" style="color:#F59E0B;font-size:11px;text-decoration:none;">businesscommandos.ru</a>' +
          '<span style="color:#475569;font-size:11px;">© Business Commandos</span>' +
        '</div>' +
  ...
  html2pdf().set({
    margin: 0,
    filename: 'checklist-ruchnoe-upravlenie.pdf',
    image: { type: 'jpeg', quality: 0.98 },
    html2canvas: { scale: 2, useCORS: true, backgroundColor: '#070B14', logging: false },
