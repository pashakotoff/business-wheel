...
dom.contactForm.addEventListener('submit', (e) => {
  ...
  if (nameValid && phoneValid && emailValid) {
    ...
    dom.smsVerification.hidden = false;
    document.getElementById('field-sms').focus();
  }
});

// SMS verification (stub)
dom.btnVerifySms.addEventListener('click', () => {
  const smsField = document.getElementById('field-sms');
  const smsError = document.getElementById('error-sms');
  const code = smsField.value.trim();
  ...
  if (/^\d{4}$/.test(code)) {
    ...
    smsError.textContent = 'Неверный код. Введите 4 цифры.';
    smsField.classList.add('form-input--error');
  }
});

dom.btnResendSms.addEventListener('click', () => {
  const smsError = document.getElementById('error-sms');
  smsError.textContent = '';
  document.getElementById('field-sms').value = '';
  document.getElementById('field-sms').focus();
});

// Back to contact form (change phone number)
document.getElementById('btn-change-phone').addEventListener('click', () => {
  dom.smsVerification.hidden = true;
  dom.contactForm.hidden = false;
  document.getElementById('field-phone').focus();
  document.getElementById('field-sms').value = '';
  document.getElementById('error-sms').textContent = '';
});
